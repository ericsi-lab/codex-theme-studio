...
启动器会自动选择通过签名检查的新版 `ChatGPT.app` 或旧版 `Codex.app`，自身不会常驻。
官方桌面应用升级后可以说“检查升级兼容性”；Skill 会验证新构建和页面结构，但不会读取
任务正文、提示词、项目名或用户名。

不要把调试地址设置为 `0.0.0.0`，也不要把端口转发给其他设备。

官方 Codex 升级为 ChatGPT 桌面端属于已识别的迁移路径，不需要用户反复下载应用，也不需要手工运行签名命令。完整签名检查以“应用版本、构建号、CDHash、可执行文件和签名资源指纹”为键缓存；只有官方应用升级或相关文件发生变化时才重新检查。

如果返回 `APP_SIGNATURE_INCOMPATIBLE`，说明应用迁移身份已经识别，但 macOS 拒绝了当前这个本机构建的签名。此时：

- 仍可列出主题、自然语言生成背景和导入主题。
- 暂停预览、应用和真实注入，不绕过安全检查。
- 等待官方桌面端更新后直接重试；重新安装只是可选修复手段，不是每次使用的固定步骤。
